    ...
    if purple_block.sum() > 0:
        ...
        for start_ind in range(0, LSR*LSB):
            ...
            if purple.size != 0: 
                ...
                purple_in_red_mat,purple_length_vec = __num_of_parts(purple,\
                                                    ri,start_red)
                ...
                purple_in_blue_mat, purple_length = __num_of_parts(purple,\
                                                    bi, start_blue)
...
def __merge_rows(input_mat, input_width):
    ...
    while rows > 0:
        ...
        if np.max(merge_block) > 1:
            (union_merge, union_merge_key) = __compare_and_cut(union_merge,\
            input_width,
...
def hierarchical_structure_with_vis(matrix_no,key_no,sn):
    """
     Same as hierarchical_structure but this also shows visualizations
    ...
    plt.title('Diagonal Matrix of the Starting Indices of Essential Structure Component (1)')
    ...
    plt.title('Diagonal Matrix of the Starting Indices of Essential Structure Component (2)')
    loc = plticker.MultipleLocator(base=1.0) # this locator puts ticks at regular intervals
    ax.yaxis.set_major_locator(loc)
    ax.xaxis.set_major_locator(loc)
    plt.show()
    ...
    plt.title('Diagonal Matrix of the Starting Indices of Essential Structure Component (3)')
    loc = plticker.MultipleLocator(base=1.0) # this locator puts ticks at regular intervals
    ax.yaxis.set_major_locator(loc)
    ax.xaxis.set_major_locator(loc)
    plt.show()
