...
def find_complete_list(pair_list,song_length):
    ...
    for j in range(1, bw_num+1):
        ...
        if add_erows.size == 0:
            if add_srows.size != 0:
                column = add_srows.shape[1]
                add_erows = np.array([],dtype=np.int64).reshape(0, column) 
                
        # Add the new pairs of repeats to the temporary list add_mat
        add_mat.extend((add_srows,add_erows,add_mrows))
        
        # Check if add_mat contains elements
        if len(add_mat) > 0: 
            combine_add_functions = list(add_srows) + list(add_erows) + list(add_mrows)
            sum_functions = sum(combine_add_functions)
            combo_array = np.array(sum_functions)
            if combo_array.sum(axis=0) > 0:
                new_mat = np.concatenate(add_mat)
                store_mat = new_mat
            else: 
                new_mat = np.array([],dtype=np.int64).reshape(0,0)
    ...
    # Reshape new_mat if it is empty
    if new_mat.size == 0:
        column = store_mat.shape[1]
        new_mat = np.array([],dtype=np.int64).reshape(0,column)
        one_mat = [store_mat, new_mat]
        new_mat = np.concatenate(one_mat)
