...
def __create_anno_remove_overlaps(k_mat, song_length, band_width):
    ...
    the function outputs k_lst_out which only contains rows that have no 
    overlaps, then takes the annotations that have overlaps from k_lst_out and
    puts them in overlap_lst. Lastly, it checks if the proper sequence of 
    annotation markers was given and fix them if necessary.
